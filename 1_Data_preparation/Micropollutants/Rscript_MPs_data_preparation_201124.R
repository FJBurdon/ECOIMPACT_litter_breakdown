...
## Calculate means
Total_TUs_median <- Total_TUs %>%
  group_by(SiteName,LMR) %>% 
  summarise_at(vars("TU_Final"),median,na.rm=T)

## Calculate means
Total_TUs_max <- Total_TUs %>%
  group_by(SiteName,LMR) %>% 
  summarise_at(vars("TU_Final"),max,na.rm=T)

## Check header
head(Total_TUs_max)
glimpse(Total_TUs_max)
...
## Calculate means
Meta4_TUs_median <- Meta4_TUs %>%
  group_by(SiteName,LMR,Meta_substance_4) %>% 
  summarise_at(vars("TU_Final"),median,na.rm=T)

## Calculate means
Meta4_TUs_max <- Meta4_TUs %>%
  group_by(SiteName,LMR,Meta_substance_4) %>% 
  summarise_at(vars("TU_Final"),max,na.rm=T)

## Separate Metal TUs
Meta4_TUs_mu_long <- Meta4_TUs_mu[which(Meta4_TUs_mu$Meta_substance_4=="Metals"),]
Meta4_TUs_median_long <- Meta4_TUs_median[which(Meta4_TUs_median$Meta_substance_4=="Metals"),]
Meta4_TUs_max_long <- Meta4_TUs_max[which(Meta4_TUs_max$Meta_substance_4=="Metals"),]

## Check header
head(Meta4_TUs_mu_long)
glimpse(Meta4_TUs_mu_long )
...
## Calculate means
Meta5_TUs_median <- Meta5_TUs %>%
  group_by(SiteName,LMR,MS5_Pesticides) %>% 
  summarise_at(vars("TU_Final"),median,na.rm=T)

## Calculate means
Meta5_TUs_max <- Meta5_TUs %>%
  group_by(SiteName,LMR,MS5_Pesticides) %>% 
  summarise_at(vars("TU_Final"),max,na.rm=T)

## Separate Pesticides etc TUs - into wide format
Meta5_TUs_mu_wide <- Meta5_TUs_mu %>% spread(MS5_Pesticides,TU_Final)
Meta5_TUs_median_wide <- Meta5_TUs_median %>% spread(MS5_Pesticides,TU_Final)
Meta5_TUs_max_wide <- Meta5_TUs_max %>% spread(MS5_Pesticides,TU_Final)

## Check header
head(Meta5_TUs_mu_wide)
glimpse(Meta5_TUs_mu_wide)
...
TU_data <- data.frame(Total_TUs_mu,Total_TUs_median[,3],Total_TUs_max[,3])
head(TU_data)
colnames(TU_data)<-c("SiteName","LMR","Total_TU_mu","Total_TU_med","Total_TU_max")

## Create dataframe for Metals data
Meta4_TUs_long <- data.frame(Meta4_TUs_mu_long[,-3],Meta4_TUs_median_long[,4],Meta4_TUs_max_long[,4])
head(Meta4_TUs_long)
colnames(Meta4_TUs_long)<-c("SiteName","LMR","Metal_TU_mu","Metal_TU_med","Metal_TU_max")

## Merge total TUs and Metal TUs
TU_data <- merge(TU_data, Meta4_TUs_long, by=c("SiteName","LMR"), all = T)

## Check header
head(TU_data)
glimpse(TU_data)

## Create dataframe for Pesticides, etc.
Meta5_TUs_wide <- data.frame(Meta5_TUs_mu_wide,Meta5_TUs_median_wide[,c(3:7)],Meta5_TUs_max_wide[,c(3:7)])
head(Meta5_TUs_wide)
colnames(Meta5_TUs_wide)<-c("SiteName","LMR","Fung_TU_mu","Herb_TU_mu","Insect_TU_mu","Other_TU_mu","Pharma_TU_mu",
                               "Fung_TU_med","Herb_TU_med","Insect_TU_med","Other_TU_med","Pharma_TU_med",
                               "Fung_TU_max","Herb_TU_max","Insect_TU_max","Other_TU_max","Pharma_TU_max")
...
TU_data_all[which(TU_data_all$SiteName=="Buttisholz"),24] <- "2013"
TU_data_all[which(TU_data_all$SiteName=="Colombier"),24] <- "2013"
#TU_data_all[which(TU_data_all$SiteName=="D<fc>rnten-Bubikon"),24] "2013"
TU_data_all[which(TU_data_all$SiteName=="Elgg"),24] <- "2014"
TU_data_all[which(TU_data_all$SiteName=="Ellikon"),24] <- "2014"
TU_data_all[which(TU_data_all$SiteName=="Herisau"),24] <- "2013"
TU_data_all[which(TU_data_all$SiteName=="Hochdorf"),24] <- "2013"
TU_data_all[which(TU_data_all$SiteName=="Hornussen"),24] <- "2013"
TU_data_all[which(TU_data_all$SiteName=="Kernenried"),24] <- "2013"
TU_data_all[which(TU_data_all$SiteName=="Knonau"),24] <- "2014"
TU_data_all[which(TU_data_all$SiteName=="Marthalen"),24] <- "2014"
TU_data_all[which(TU_data_all$SiteName=="Messen"),24] <- "2013"
TU_data_all[which(TU_data_all$SiteName=="Muri"),24] <- "2014"
TU_data_all[which(TU_data_all$SiteName=="Niederdorf"),24] <- "2013"
TU_data_all[which(TU_data_all$SiteName=="Reinach"),24] <- "2014"
TU_data_all[which(TU_data_all$SiteName=="Romont"),24] <- "2013"
TU_data_all[which(TU_data_all$SiteName=="Rothenturm"),24] <- "2013"
#TU_data_all[which(TU_data_all$SiteName=="S<fc>very"),1]0 <- "2013"
TU_data_all[which(TU_data_all$SiteName=="Val-de-Ruz"),24] <- "2014"
TU_data_all[which(TU_data_all$SiteName=="Villeret"),24] <- "2014"
...
TU_data_all <- TU_data_all[,c(24,1:23)]
head(TU_data_all)
...
## Shows that metasubstances 5 (Fung,Herb,Insect,Other,Pharma) sum to total TUs
plot(log(rowSums(TU_data_all[,c(10:14)])) ~ log(TU_data_all[,4]))
abline(0,1,col="blue")
...
## Check Total TUs
plot(log(TU_data_all$Total_TU_mu)~log(MPs_pivot$TUs))
abline(0,1,col="blue")
resid(lm(log(TU_data_all$Total_TU_mu)~log(MPs_pivot$TUs)))

## Check Metal TUs
plot(log(TU_data_all$Metal_TU_mu)~log(MPs_pivot$TU_avg_Metals_MS4))
abline(0,1,col="blue")

## Check Fungicide TUs
plot(log(TU_data_all$Fung_TU_mu)~log(MPs_pivot$TU_avg_Fungicide_MS5))
abline(0,1,col="blue")

## Check Herbicide TUs
plot(log(TU_data_all$Herb_TU_mu)~log(MPs_pivot$TU_avg_Herbicide_MS5))
abline(0,1,col="blue")

## Check Insecticide TUs
plot(log(TU_data_all$Insect_TU_mu)~log(MPs_pivot$TU_avg_Insecticide_MS5))
abline(0,1,col="blue")
...
resid(lm(log(TU_data_all$Insect_TU_mu)~log(MPs_pivot$TU_avg_Insecticide_MS5)))

## Check Other TUs
plot(log(TU_data_all$Other_TU_mu)~log(MPs_pivot$TU_avg_Other_MS5)) 
abline(0,1,col="blue")

## Check Pharmaceutical TUs
plot(log(TU_data_all$Pharma_TU_mu)~log(MPs_pivot$TU_avg_Pharma_MS5)) 
abline(0,1,col="blue")
...
glimpse(MP_Ecoimpact_invert_TUs)
MP_Ecoimpact_invert_TUs <- MP_Ecoimpact_invert_TUs[-which(MP_Ecoimpact_invert_TUs$Location=="U2"),]
MP_Ecoimpact_invert_TUs_U1 <- MP_Ecoimpact_invert_TUs[which(MP_Ecoimpact_invert_TUs$Location=="U1"),]
MP_Ecoimpact_invert_TUs_U1$Location <- "U2"
MP_Ecoimpact_invert_TU_final <- rbind(MP_Ecoimpact_invert_TUs,MP_Ecoimpact_invert_TUs_U1)
MP_Ecoimpact_invert_TU_final <- arrange(MP_Ecoimpact_invert_TU_final, Year, Site, Location)
write.csv(MP_Ecoimpact_invert_TU_final,"MP_Ecoimpact_invert_TU_final.csv",row.names = T)
...
min(TU_data_Ecoimpact$Insect_TU_mu)
plot(log(Insect_TU_mu) ~ log(sumTU_PB), data=TU_data_Ecoimpact)
abline(lm(log(Insect_TU_mu) ~ log(sumTU_PB), data=TU_data_Ecoimpact),col="red")
abline(0,1,col="blue")
summary(lm(log(Insect_TU_mu) ~ log(sumTU_PB), data=TU_data_Ecoimpact))

## Geometric mean sum Insecticides vs "log_TU_PB_U1"??
min(TU_data_Ecoimpact$log_TU_PB_U1)
min(TU_data_Ecoimpact$Insect_TU_mu)
plot(log(Insect_TU_mu) ~ log_TU_PB_U1, data=TU_data_Ecoimpact)
abline(lm(log(Insect_TU_mu) ~ log_TU_PB_U1, data=TU_data_Ecoimpact),col="red")
abline(0,1,col="blue")
summary(lm(log(Insect_TU_mu) ~ log_TU_PB_U1, data=TU_data_Ecoimpact))

## Geometric mean sum Insecticides vs max sum Pesticides and Biocides
min(TU_data_Ecoimpact$maxTU_PB)
min(TU_data_Ecoimpact$Insect_TU_mu)
plot(log(Insect_TU_mu) ~ log(maxTU_PB), data=TU_data_Ecoimpact)
abline(lm(log(Insect_TU_mu) ~ log(maxTU_PB), data=TU_data_Ecoimpact),col="red")
abline(0,1,col="blue")
summary(lm(log(Insect_TU_mu) ~ log(maxTU_PB), data=TU_data_Ecoimpact))

## Geometric mean sum Insecticides vs "log_maxTU_PB"??
min(TU_data_Ecoimpact$log_maxTU_PB)
min(TU_data_Ecoimpact$Insect_TU_mu)
plot(log(Insect_TU_mu) ~ log_maxTU_PB, data=TU_data_Ecoimpact)
abline(lm(log(Insect_TU_mu) ~ log_maxTU_PB, data=TU_data_Ecoimpact),col="red")
abline(0,1,col="blue")
summary(lm(log(Insect_TU_mu) ~ log_maxTU_PB, data=TU_data_Ecoimpact))
...
## Reload data with correct site info
MP_Ecoimpact_sumTU_msPAF_U2 <- MP_Ecoimpact_sumTU_msPAF[which(MP_Ecoimpact_sumTU_msPAF$Location=="U1"),]
MP_Ecoimpact_sumTU_msPAF_U2$Location <- "U2"
MP_Ecoimpact_sumTU_msPAF_final <- rbind(MP_Ecoimpact_sumTU_msPAF,MP_Ecoimpact_sumTU_msPAF_U2)
MP_Ecoimpact_sumTU_msPAF_final <- arrange(MP_Ecoimpact_sumTU_msPAF_final, Year, Site, Location)
write.csv(MP_Ecoimpact_sumTU_msPAF_final,"MP_Ecoimpact_sumTU_msPAF_final.csv",row.names = F)
...
min(TU_data_Ecoimpact$Insect_TU_mu)
plot(log(Insect_TU_mu) ~ log(median_msPAFraMix_PB), data=TU_data_Ecoimpact)
abline(lm(log(Insect_TU_mu) ~ log(median_msPAFraMix_PB), data=TU_data_Ecoimpact),col="red")
summary(lm(log(Insect_TU_mu) ~ log(median_msPAFraMix_PB), data=TU_data_Ecoimpact))

## Geometric mean sum Insecticides vs "median_msPAFraMix_I" Insecticides
min(TU_data_Ecoimpact$median_msPAFraMix_I)
min(TU_data_Ecoimpact$Insect_TU_mu)
plot(sqrt(Insect_TU_mu) ~ sqrt(median_msPAFraMix_I), data=TU_data_Ecoimpact)
abline(lm(sqrt(Insect_TU_mu) ~ sqrt(median_msPAFraMix_I), data=TU_data_Ecoimpact),col="red")
summary(lm(sqrt(Insect_TU_mu) ~ sqrt(median_msPAFraMix_I), data=TU_data_Ecoimpact))
...
min(TU_data_Ecoimpact$Insect_TU_med)
plot(sqrt(Insect_TU_med) ~ sqrt(median_sumTU_I.DM_gm), 
     xlab="sqrt median_sumTU_I.DM_gm (Munz et al. 2017)",
     ylab="sqrt Median sum TUs Insecticides (Burdon et al. in prep)",
     main="Insecticides TUs Daphnia magna",
     data=TU_data_Ecoimpact)
abline(lm(sqrt(Insect_TU_med) ~ sqrt(median_sumTU_I.DM_gm), data=TU_data_Ecoimpact),col="red")
abline(0,1,col="blue")
summary(lm(sqrt(Insect_TU_med) ~ sqrt(median_sumTU_I.DM_gm), data=TU_data_Ecoimpact))
...
## Create plot for interpretation
png(filename="ECOIMPACT_Insecticides_TUs_median_updated.png", 
    type="cairo",
    units="in", 
    width=10, 
    height=7, 
...
min(TU_data_Ecoimpact$Insect_TU_med)
plot(sqrt(Insect_TU_med) ~ sqrt(median_sumTU_I_gm), 
     xlab="sqrt median_sumTU_I_gm (Munz et al. 2017)",
     ylab="sqrt median sum TUs Insecticides (Burdon et al. in prep)",
     main="Insecticides TUs (All - Munz et al. 2017)",
     data=TU_data_Ecoimpact)
abline(lm(sqrt(Insect_TU_med) ~ sqrt(median_sumTU_I_gm), data=TU_data_Ecoimpact),col="red")
abline(0,1,col="blue")
summary(lm(sqrt(Insect_TU_med) ~ sqrt(median_sumTU_I_gm), data=TU_data_Ecoimpact))
...
min(TU_data_Ecoimpact$Insect_TU_mu)
plot(sqrt(Insect_TU_mu) ~ log(SPEAR), 
     xlab="log SPEAR (Munz et al. 2017)",
     ylab="log mean sum TUs Insecticides (Burdon et al. in prep)",
     main="Insecticides TUs Daphnia magna",
     data=TU_data_Ecoimpact)
abline(lm(sqrt(Insect_TU_mu) ~ log(SPEAR), data=TU_data_Ecoimpact),col="red")
summary(lm(sqrt(Insect_TU_mu) ~ log(SPEAR), data=TU_data_Ecoimpact))

## Geometric mean sum Insecticides vs max sum Pesticides and Biocides
min(TU_data_Ecoimpact$SPEAR)
min(TU_data_Ecoimpact$Insect_TU_med)
plot(sqrt(Insect_TU_med) ~ log(SPEAR), 
     xlab="log SPEAR (Munz et al. 2017)",
     ylab="log median sum TUs Insecticides (Burdon et al. in prep)",
     main="Insecticides TUs Daphnia magna",
     data=TU_data_Ecoimpact)
abline(lm(sqrt(Insect_TU_med) ~ log(SPEAR), data=TU_data_Ecoimpact),col="red")
summary(lm(sqrt(Insect_TU_med) ~ log(SPEAR), data=TU_data_Ecoimpact))

## Geometric mean sum Insecticides vs max sum Pesticides and Biocides
min(TU_data_Ecoimpact$SPEAR)
min(TU_data_Ecoimpact$Insect_TU_max)
plot(sqrt(Insect_TU_max) ~ log(SPEAR), 
     xlab="log SPEAR (Munz et al. 2017)",
     ylab="log maximum sum TUs Insecticides (Burdon et al. in prep)",
     main="Insecticides TUs Daphnia magna",
     data=TU_data_Ecoimpact)
abline(lm(sqrt(Insect_TU_max) ~ log(SPEAR), data=TU_data_Ecoimpact),col="red")
summary(lm(sqrt(Insect_TU_max) ~ log(SPEAR), data=TU_data_Ecoimpact))
...
## Create plot for interpretation
png(filename="ECOIMPACT_Insecticides_TUs_mean_median_max.png", 
    type="cairo",
    units="in", 
    width=8.75, 
    height=6, 
...
## Geometric mean sum Insecticides vs max sum Pesticides and Biocides
plot(sqrt(Insect_TU_mu) ~ sqrt(Insect_TU_med), 
     xlab="sqrt median sum TUs Insecticides (Burdon et al. in prep)",
     ylab="sqrt mean sum TUs Insecticides (Burdon et al. in prep)",
     main="Insecticides TUs Daphnia magna",
     data=TU_data_Ecoimpact)
abline(0,1,col="blue")

## Geometric mean sum Insecticides vs max sum Pesticides and Biocides
plot(sqrt(Insect_TU_max) ~ sqrt(Insect_TU_med), 
     xlab="sqrt median sum TUs Insecticides (Burdon et al. in prep)",
     ylab="sqrt max sum TUs Insecticides (Burdon et al. in prep)",
     main="Insecticides TUs Daphnia magna",
     data=TU_data_Ecoimpact)
abline(0,1,col="blue")

## Geometric mean sum Insecticides vs max sum Pesticides and Biocides
plot(sqrt(Insect_TU_max) ~ sqrt(Insect_TU_mu), 
     xlab="sqrt mean sum TUs Insecticides (Burdon et al. in prep)",
     ylab="sqrt max sum TUs Insecticides (Burdon et al. in prep)",
     main="Insecticides TUs Daphnia magna",
     data=TU_data_Ecoimpact)
abline(0,1,col="blue")
