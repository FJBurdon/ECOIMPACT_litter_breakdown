...
write.csv(CWM_traits,file = "lpa_alder_coarse_allyrs_CWM_traits_data.csv",row.names = F)

## Write CMW (community weighted means) for all leafpacks (includes zeros)
head(CWM_traits[,c(1:8)])
all_leafpacks_traits <- merge(invert_abundance_out[,c(1:8)],CWM_traits[,c(2,3,6,9:121)],by=c("Site_code","Location","Rep"),all.x = T)
all_leafpacks_traits [is.na(all_leafpacks_traits)] <- 0
write.csv(all_leafpacks_traits,"lpa_alder_coarse_allyrs_CWM_traits_data_all_leapacks.csv",row.names = F)
